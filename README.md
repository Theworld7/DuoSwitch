...
Windows 自带的「设置 → 个性化 → 颜色」里也有「自动在浅色和深色间切换」。
**建议在那里关掉**，由 DuoSwitch 统一调度。

如果忘了关、本时段主题被系统改掉，DuoSwitch 会把壁纸跟着系统主题走，并
在界面顶部弹一条提示告诉你。强行夺回会和系统调度互相翻转成死循环，所以
默认不夺回。
...
仓库内置的示例壁纸来自 **macOS Sonoma**，版权归 **Apple Inc.** 所有，
仅作为应用功能演示使用。如需替换成自己的图，请直接在应用界面
「浅色 / 深色模式壁纸」处点「选择图片」。
